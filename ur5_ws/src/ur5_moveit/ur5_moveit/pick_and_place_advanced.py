...
class PickAndPlace(Node):
    def __init__(self):
        ...
        # Place positions - 3 spots on the place table for each cube
        # Place table top at z=0.30
        self.place_positions = [
            {'name': 'place_1', 'x': 0.35, 'y': -0.45, 'z': 0.34},
            {'name': 'place_2', 'x': 0.45, 'y': -0.45, 'z': 0.34},
            {'name': 'place_3', 'x': 0.40, 'y': -0.55, 'z': 0.34},
        ]
    ...
    def remove_collision_object(self, name):
        ...
        self.collision_pub.publish(obj)
        self.get_logger().info(f'Removed collision: {name}')
    ...
    def move_to_pose(self, pose: Pose, desc: str) -> bool:
        ...
        for i, sol in enumerate(solutions, 1):
            ...
            if self.obstacle_detected:
                self.get_logger().info('  Obstacle detected, trying next configuration...')
                self.obstacle_detected = False
    ...
    def place(self, cube_name: str, place_pos: dict) -> bool:
        x, y, z = place_pos['x'], place_pos['y'], place_pos['z']
        
        self.get_logger().info(f'')
        self.get_logger().info(f'=== PLACING {cube_name} at {place_pos["name"]} ===')
    ...
    def run(self):
        ...
        self.get_logger().info('=' * 60)
        self.get_logger().info('PICK AND PLACE DEMO - ALL 3 CUBES')
        self.get_logger().info('Insert obstacles anytime to test replanning!')
        self.get_logger().info('=' * 60)
        ...
        for i, cube in enumerate(self.cube_positions):
            self.get_logger().info('')
            self.get_logger().info('*' * 60)
            self.get_logger().info(f'CUBE {i+1} of {len(self.cube_positions)}: {cube["name"]}')
            self.get_logger().info('*' * 60)
            
            if self.pick(cube):
                if self.place(cube['name'], self.place_positions[i]):
                    successful += 1
                    self.get_logger().info(f'{cube["name"]} completed successfully!')
                else:
                    failed += 1
                    self.get_logger().error(f'Failed to place {cube["name"]}')
                    self.open_gripper()
            else:
                failed += 1
                self.get_logger().error(f'Failed to pick {cube["name"]}')
            
            if i < len(self.cube_positions) - 1:
                self.get_logger().info('')
                self.get_logger().info('--- Moving to next cube in 2 seconds ---')
                time.sleep(2.0)
        
        self.get_logger().info('')
        self.get_logger().info('=' * 60)
        self.get_logger().info('DEMO COMPLETE!')
        self.get_logger().info(f'Successful: {successful}/{len(self.cube_positions)}')
        self.get_logger().info(f'Failed: {failed}/{len(self.cube_positions)}')
        self.get_logger().info('=' * 60)
